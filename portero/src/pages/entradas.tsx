...
const Entradas = () => {
    ...
    const columnasEntradas: GridColDef[] = [
        ...
        { field: 'fecha_entrada', headerName: 'Fecha Entrada', width: 200, renderCell: (params) => dayjs(params.value).format('DD/MM/YYYY HH:mm') },
    ];

    const exportToPDF = () => {
        const doc = new jsPDF();
        doc.text("Reporte de Entradas Activas", 14, 16);

        // Tabla de Hoy
        doc.text("Entradas de Hoy", 14, 26);
        autoTable(doc, {
            startY: 30,
            head: [['Usuario', 'Equipo', 'Descripción', 'Fecha Entrada']],
            body: todaysEntries.map(entry => [
                entry.usuarioNombre,
                entry.equipoNombre,
                entry.descripcion,
                dayjs(entry.fecha_entrada).format('DD/MM/YYYY HH:mm')
            ]),
        });

        // Tabla de Otras Fechas
        const finalY = (doc as any).lastAutoTable.finalY || 30;
        doc.text("Entradas de Otras Fechas", 14, finalY + 10);
        autoTable(doc, {
            startY: finalY + 14,
            head: [['Usuario', 'Equipo', 'Descripción', 'Fecha Entrada']],
            body: otherEntries.map(entry => [
                entry.usuarioNombre,
                entry.equipoNombre,
                entry.descripcion,
                dayjs(entry.fecha_entrada).format('DD/MM/YYYY HH:mm')
